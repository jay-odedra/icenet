...
# GLOBALS
from configs.brkprime.mvavars import *
from configs.brkprime.cuts import *
from configs.brkprime.filter import *
...
def load_root_file(root_path, ids=None, entry_start=0, entry_stop=None, maxevents=None, args=None):
    ...
    X_DATA_e1 = X_DATA[:, [VARS_DATA.index(name.replace("x_", "e1_")) for name in NEW_VARS]]
    Y_DATA_e1 = np.zeros(X_DATA.shape[0], dtype=int)
    
    X_DATA, VARS_DATA = process_root(rootfile=rootfile, tree='tree', isMC='data', **param)

    X_DATA_e2 = X_DATA[:, [VARS_DATA.index(name.replace("x_", "e2_")) for name in NEW_VARS]]
    Y_DATA_e2 = np.zeros(X_DATA.shape[0], dtype=int)
    
    # =================================================================
    # Finally combine

    X = np.concatenate((X_MC_e1, X_MC_e2, X_DATA_e1, X_DATA_e2), axis=0)
    Y = np.concatenate((Y_MC_e1, Y_MC_e2, Y_DATA_e1, Y_DATA_e2), axis=0)
